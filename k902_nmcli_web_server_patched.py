...
def _has_nmcli() -> bool:
    try:
        subprocess.run(["nmcli", "-v"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
        return True
...
def ensure_hotspot(ssid: str = \"Hotspot\", password: str = \"12345678\", ifname: str = \"wlan0\", wifi_backend: str = \"nmcli\", wifi_backend=args.wifi) -> None:
    ...
    if wifi_backend != \"nmcli\":
        print(\"Hotspot: gestionado externamente (hostapd/dnsmasq).\")
        return
    if not _has_nmcli():
        print(\"Hotspot: nmcli no está disponible; omitiendo configuración.\")
        return
...
def connect_to_wifi(ssid: str, password: str, ifname: str = \"wlan0\", wifi_backend: str = \"nmcli\") -> bool:
    """Connect to a Wi‑Fi network using nmcli. Returns True on success."""
    if wifi_backend != \"nmcli\" or not _has_nmcli():
        print(\"Conectar Wi-Fi: deshabilitado (sin nmcli).\")
        return False
    try:
...
def scan_wifi(ifname: str = \"wlan0\", wifi_backend: str = \"nmcli\") -> List[str]:
...
""" + body + "\n</body></html>").encode()
    ...
    def do_GET(self) -> None:
        ...
        seleccionado.
        """
        nets = scan_wifi(wifi_backend=getattr(self, \"wifi_backend\", \"nmcli\"))
    ...
    def do_POST(self) -> None:
        ...
        if mode == 'rover':
            ...
            if not ssid:
                ...
                if connect_to_wifi(ssid, wifipass, wifi_backend=getattr(self, \"wifi_backend\", \"nmcli\")):
...
class ConfigHTTPServer:
    def __init__(self, ser: serial.Serial, host: str = \"0.0.0.0\", port: int = 8080, wifi_backend: str = \"nmcli\") -> None:
        self.ser = ser
        self.host = host
        self.port = port
        handler_class = ConfigHTTPRequestHandler
        handler_class.wifi_backend = wifi_backend
...
def main() -> None:
    ...
    parser.add_argument(\"--wifi\", choices=[\"nmcli\",\"none\"], default=\"nmcli\",
                        help=\"Gestor Wi-Fi: 'nmcli' o 'none' (no gestionar Wi-Fi)\")
    ...
    http_server = ConfigHTTPServer(ser, wifi_backend=args.wifi)
